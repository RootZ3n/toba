#!/usr/bin/env bash
# Cursus Tailscale-readiness verification
# ========================================
...
#   CURSUS_URL=http://100.64.0.1:18815 CURSUS_AUTH_TOKEN=... ./scripts/verify-tailscale-ready.sh
#
# It exercises:
...
#   - /cursus/provider does not leak API keys (api_key_set boolean only)
#   - GET /cursus/dux/agents lists the registry
#   - if a token is set, sensitive endpoints reject without auth and allow with auth

set -uo pipefail

CURSUS_URL="${CURSUS_URL:-http://127.0.0.1:18815}"
AUTH_TOKEN="${CURSUS_AUTH_TOKEN:-}"
AUTH_HDR=()
...
echo "── Cursus Tailscale readiness ────────────────────────────────────────"
echo "Target: $CURSUS_URL  (token: $([ -n "$AUTH_TOKEN" ] && echo set || echo unset))"
echo ""

echo "[1] Public endpoints"
check "GET /health responds 200"   '[ "$(curl -fsS -o /dev/null -w "%{http_code}" '"$CURSUS_URL"'/health)" = "200" ]'
check "GET /version responds 200"  '[ "$(curl -fsS -o /dev/null -w "%{http_code}" '"$CURSUS_URL"'/version)" = "200" ]'

echo ""
echo "[2] /status shape"
status_json="$(curl -fsS "${AUTH_HDR[@]}" "$CURSUS_URL/status")"
export status_json
...
prov_json="$(curl -fsS "${AUTH_HDR[@]}" "$CURSUS_URL/cursus/provider")"
export prov_json
check "/cursus/provider: api_key_set is boolean, key never present" \
...
agents_json="$(curl -fsS "${AUTH_HDR[@]}" "$CURSUS_URL/cursus/dux/agents")"
export agents_json
check "GET /cursus/dux/agents ok=true"        '[ "$(echo "$agents_json" | jq -r .ok)" = "true" ]'
...
if [ "$exposure" != "loopback_only" ]; then
  ...
  if [ -n "$AUTH_TOKEN" ]; then
    ...
    NO_AUTH_STATUS="$(curl -fsS -o /dev/null -w "%{http_code}" "$CURSUS_URL/cursus/profile")"
    check "unauthenticated /cursus/profile -> 401 OR 200 (depending on connection origin)" \
      '[ "'"$NO_AUTH_STATUS"'" = "401" ] || [ "'"$NO_AUTH_STATUS"'" = "200" ]'
    WITH_AUTH_STATUS="$(curl -fsS -o /dev/null -w "%{http_code}" "${AUTH_HDR[@]}" "$CURSUS_URL/cursus/profile")"
    check "/cursus/profile with token -> 200"            '[ "'"$WITH_AUTH_STATUS"'" = "200" ]'
  fi
else
  echo "[5b] Loopback only — to enable Tailscale, set CURSUS_HOST=0.0.0.0 (or a Tailscale IP) and CURSUS_AUTH_TOKEN"
fi
