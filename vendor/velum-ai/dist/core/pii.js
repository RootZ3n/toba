...
function logDetections(level, detections, masked) {
    if (detections.length === 0)
        return;
    detectionLog.push({ timestamp: new Date().toISOString(), level, detections, masked });
    while (detectionLog.length > MAX_LOG_ENTRIES)
        detectionLog.shift();
}
// ── False-positive guards (M5) ───────────────────────────────────────────────
...
function isLikelyName(source, d) {
 ...
 */
export function scanPii(text, registry = defaultRegistry, options = {}) {
    const source = text ?? "";
    const raw = [];
    ...
    const filtered = raw.filter((d) => {
        if (d.type === "CREDIT_CARD")
            return passesLuhn(d.value);
        if (d.type === "NAME")
            return options.detectNames === true && isLikelyName(source, d);
        return true;
    });
...
export function maskPii(text, registry = defaultRegistry, options = {}) {
    const source = text ?? "";
...
export function demask(text, placeholderMap) {
    let result = text ?? "";
...
export function sanitizePii(text, registry = defaultRegistry, options = {}) {
    const source = text ?? "";
    const detections = scanPii(source, registry, options); // descending by start
    let result = source;
    for (const d of detections) {
...
export function processWithPii(text, level, registry = defaultRegistry, options = {}) {
