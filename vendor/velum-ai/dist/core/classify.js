...
const CREDENTIAL_WARNING = "Credential detected and secured. If this was for a tool setup, continue with your request and the tool will retrieve it automatically.";
// Maps an injection pattern name → the classification it implies.
const INJECTION_CLASSIFICATION = {
...
const SEVERITY_RANK = { warn: 1, review: 2, block: 3 };
/**
 ...
 */
function isSafeMatch(value, neverRedact) {
...
function detectInjection(text, registry) {
...
function testNormalized(pattern, normalized) {
    pattern.lastIndex = 0;
    const r = pattern.test(normalized);
    pattern.lastIndex = 0;
    return r;
}
export function classify(message, sessionId, options = {}) {
    ...
    for (const def of registry.credentialPatterns) {
        ...
        const realMatches = def.confidence === "high"
            ? matches
    ...
    if (foundCredential) {
        ...
        if (inj.patterns.length > 0) {
            for (const p of inj.patterns)
                if (!patternsMatched.includes(p))
                    patternsMatched.push(p);
