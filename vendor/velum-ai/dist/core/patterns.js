...
export const DEFAULT_NEVER_REDACT = [
...
class VelumPatternRegistry {
    ...
    addPattern(def) {
        ...
        if ((def.category === "credential" || def.category === "pii") && !pattern.flags.includes("g")) {
            ...
            console.warn(`[velum] pattern '${def.name}' (${def.category}) was missing the 'g' flag; auto-adding it to prevent scan hangs.`);
 ...
 */
export function ensureGlobal(re) {
