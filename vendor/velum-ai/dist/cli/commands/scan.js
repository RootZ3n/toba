...
function walk(path, out) {
    ...
    if (st.isDirectory()) {
        ...
        if (st.size <= MAX_FILE_BYTES && (TEXT_EXT.test(path) || !path.includes(".")))
            out.push(path);
    }
}
function maskPreview(line, match) {
...
function scanTextContent(file, text) {
    const findings = [];
...
function printTable(findings) {
    ...
    if (blocking > 0)
        process.stdout.write(`✗ ${blocking} blocking finding(s).\n`);
}
function readStdin() {
